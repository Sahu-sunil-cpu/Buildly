...
export interface StructuredfileSchema {
    name: string,
    language: string,
    value: string
}
...
export function* parseBoltArtifact(input: string) {
    const FilePath = [];
    const Structuredfile: StructuredfileSchema[] = []




    // Match everything inside <boltAction ...>...</boltAction>
    const fileRegex = /<boltAction[^>]*filePath="([^"]+)"[^>]*>([\s\S]*?)<\/boltAction>/g;
...
function extractHiddenFiles(input: string) {
    const match = input.match(/Here is a list of files.*?:\n\n([\s\S]+)$/);
    if (!match) return [];
